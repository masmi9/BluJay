#!/usr/bin/env python3
"""
improper_platform_usage - Structured Result Bridge (V2 Phase 2)
====================================================================
...
class ImproperPlatformUsageV2(BasePluginV2):
    """Platform usage analysis - permissions, components, root bypass (CWE-926/693)"""

    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="improper_platform_usage",
            version="3.0.0",
            description="Platform usage analysis - permissions, components, root bypass (CWE-926/693)",
            author="AODS Team",
            capabilities=[PluginCapability.STATIC_ANALYSIS, PluginCapability.VULNERABILITY_DETECTION],
            priority=PluginPriority.NORMAL,
            timeout_seconds=120,
            supported_platforms=["android"],
            tags=["platform", "permissions", "masvs-platform-1", "cwe-926"],
        )
    ...
    def execute(self, apk_ctx) -> PluginResult:
        ...
        try:
            ...
            logger.error("improper_platform_usage failed: %s", e)
            return PluginResult(
    ...
    def _run_structured(self, apk_ctx) -> List[PluginFinding]:
        ...
        try:
            ...
            result = plugin._perform_enhanced_analysis()
            ...
            try:
                from plugins.improper_platform_usage import run_plugin
                run_plugin(apk_ctx)
            except Exception:
                pass
            return []
    ...
    def _convert_finding(self, v, index: int) -> PluginFinding:
        ...
        if isinstance(v, dict):
            ...
            return PluginFinding(
                finding_id=f"improper_platform_usage_{index:03d}",
                title=str(v)[:200],
        ...
        if not cwe:
            tl = (title + " " + desc).lower()
            if any(k in tl for k in ["exported", "component"]):
                cwe = "CWE-926"
            elif any(k in tl for k in ["root", "bypass"]):
                cwe = "CWE-693"
            elif any(k in tl for k in ["permission"]):
                cwe = "CWE-732"
...
def create_plugin():
    return ImproperPlatformUsageV2()


__all__ = ["ImproperPlatformUsageV2", "create_plugin"]
