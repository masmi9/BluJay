#!/usr/bin/env python3
"""
network_cleartext_traffic - Structured Result Bridge (V2 Phase 2)
======================================================================
...
class NetworkCleartextTrafficV2(BasePluginV2):
    """Cleartext traffic detection - manifest flags, NSC, hardcoded HTTP URLs (CWE-319)"""

    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="network_cleartext_traffic",
            version="3.0.0",
            description="Cleartext traffic detection - manifest flags, NSC, hardcoded HTTP URLs (CWE-319)",
            author="AODS Team",
            capabilities=[PluginCapability.STATIC_ANALYSIS, PluginCapability.VULNERABILITY_DETECTION],
            priority=PluginPriority.NORMAL,
            timeout_seconds=120,
            supported_platforms=["android"],
            tags=["network", "cleartext", "masvs-network-1", "cwe-319"],
        )
    ...
    def execute(self, apk_ctx) -> PluginResult:
        ...
        try:
            ...
            logger.error("network_cleartext_traffic failed: %s", e)
            return PluginResult(
    ...
    def _run_structured(self, apk_ctx) -> List[PluginFinding]:
        ...
        try:
            from plugins.network_cleartext_traffic import NetworkCleartextTrafficAnalyzer

            plugin = NetworkCleartextTrafficAnalyzer(apk_ctx)
            result = plugin.analyze()

            # Extract findings/vulnerabilities from result
            raw = getattr(result, "findings", None) or []
            if not raw:
            ...
            try:
                from plugins.network_cleartext_traffic import run_plugin
                run_plugin(apk_ctx)
            except Exception:
                pass
            return []
    ...
    def _convert_finding(self, v, index: int) -> PluginFinding:
        ...
        if isinstance(v, dict):
            ...
            return PluginFinding(
                finding_id=f"network_cleartext_traffic_{index:03d}",
                title=str(v)[:200],
        ...
        if not cwe:
            tl = (title + " " + desc).lower()
            if any(k in tl for k in ["cleartext", "http"]):
                cwe = "CWE-319"
            elif any(k in tl for k in ["nsc", "security config"]):
                cwe = "CWE-693"
...
def create_plugin():
    return NetworkCleartextTrafficV2()


__all__ = ["NetworkCleartextTrafficV2", "create_plugin"]
