#!/usr/bin/env python3
"""
enhanced_network_security_analysis - V2 Structured Bridge (Phase 3)
========================================================================
"""
...
class EnhancedNetworkSecurityAnalysisV2(BasePluginV2):
    """Network security config analysis - NSC, TLS issues, cert bypass (CWE-295)"""

    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="enhanced_network_security_analysis", version="3.0.0",
            description="Network security config analysis - NSC, TLS issues, cert bypass (CWE-295)",
            author="AODS Team",
            ...
            supported_platforms=["android"], tags=["network", "nsc", "cwe-295"],
        )
    ...
    def execute(self, apk_ctx) -> PluginResult:
        ...
        try:
            ...
            logger.error("enhanced_network_security_analysis failed: %s", e)
    ...
    def _run_structured(self, apk_ctx):
        try:
            from plugins.enhanced_network_security_analysis import EnhancedNetworkSecurityAnalyzer
            plugin = EnhancedNetworkSecurityAnalyzer(apk_ctx)
            ...
            logger.debug("Structured failed for enhanced_network_security_analysis: %s", e)
            return self._run_legacy(apk_ctx)

    def _run_legacy(self, apk_ctx):
        try:
            from plugins.enhanced_network_security_analysis import run_plugin
            ...
            logger.debug("Legacy failed for enhanced_network_security_analysis: %s", e)
            return []
        ...
        for i, v in enumerate(items):
            if isinstance(v, str):
                findings.append(PluginFinding(
                    finding_id=f"enhanced_network_security_analysis_{i:03d}", title=v[:200], description=v[:500],
                ...
                if sev not in ("critical", "high", "medium", "low", "info"):
                    sev = "medium"
                findings.append(PluginFinding(
                    finding_id=f"enhanced_network_security_analysis_{i:03d}",
                ...
                findings.append(PluginFinding(
                    ...
                    cwe_id=getattr(v, "cwe_id", "CWE-693"),
...
def create_plugin():
    return EnhancedNetworkSecurityAnalysisV2()


__all__ = ["EnhancedNetworkSecurityAnalysisV2", "create_plugin"]
