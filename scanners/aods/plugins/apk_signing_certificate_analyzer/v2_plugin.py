#!/usr/bin/env python3
"""
apk_signing_certificate_analyzer - Structured Result Bridge (V2 Phase 2)
=============================================================================
...
class APKSigningCertificateAnalyzerV2(BasePluginV2):
    """APK signing certificate analysis - scheme, key strength, chain validation (CWE-347)"""

    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="apk_signing_certificate_analyzer",
            version="3.0.0",
            description="APK signing certificate analysis - scheme, key strength, chain validation (CWE-347)",
            author="AODS Team",
            capabilities=[PluginCapability.STATIC_ANALYSIS, PluginCapability.VULNERABILITY_DETECTION],
            priority=PluginPriority.NORMAL,
            timeout_seconds=120,
            supported_platforms=["android"],
            tags=["signing", "certificate", "cwe-347"],
        )
    ...
    def execute(self, apk_ctx) -> PluginResult:
        ...
        try:
            ...
            logger.error("apk_signing_certificate_analyzer failed: %s", e)
            return PluginResult(
    ...
    def _run_structured(self, apk_ctx) -> List[PluginFinding]:
        ...
        try:
            from plugins.apk_signing_certificate_analyzer import APKSigningCertificateAnalyzer

            plugin = APKSigningCertificateAnalyzer(apk_ctx)
            result = plugin.analyze_apk_signing_certificates()
            ...
            try:
                from plugins.apk_signing_certificate_analyzer import run_plugin
                run_plugin(apk_ctx)
            except Exception:
                pass
            return []
    ...
    def _convert_finding(self, v, index: int) -> PluginFinding:
        ...
        if isinstance(v, dict):
            ...
            return PluginFinding(
                finding_id=f"apk_signing_certificate_analyzer_{index:03d}",
                title=str(v)[:200],
...
def create_plugin():
    return APKSigningCertificateAnalyzerV2()


__all__ = ["APKSigningCertificateAnalyzerV2", "create_plugin"]
