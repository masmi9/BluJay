#!/usr/bin/env python3
"""
advanced_ssl_tls_analyzer - Structured Result Bridge (V2 Phase 2)
======================================================================
...
class AdvancedSSLTLSAnalyzerV2(BasePluginV2):
    """SSL/TLS security analysis with cert validation, protocol, cipher checks (CWE-295/327)"""

    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="advanced_ssl_tls_analyzer",
            version="3.0.0",
            description="SSL/TLS security analysis with cert validation, protocol, cipher checks (CWE-295/327)",
            author="AODS Team",
            capabilities=[PluginCapability.STATIC_ANALYSIS, PluginCapability.VULNERABILITY_DETECTION],
            priority=PluginPriority.NORMAL,
            timeout_seconds=120,
            supported_platforms=["android"],
            tags=["ssl", "tls", "masvs-network-1", "cwe-295"],
        )
    ...
    def execute(self, apk_ctx) -> PluginResult:
        ...
        try:
            ...
            logger.error("advanced_ssl_tls_analyzer failed: %s", e)
            return PluginResult(
    ...
    def _run_structured(self, apk_ctx) -> List[PluginFinding]:
        ...
        try:
            ...
            plugin = AdvancedSSLTLSAnalyzerPlugin(apk_ctx)
            result = plugin.analyze_ssl_tls_security()

            # Extract findings/vulnerabilities from result
            raw = getattr(result, "vulnerabilities", None) or []
            if not raw:
            ...
            try:
                from plugins.advanced_ssl_tls_analyzer import run_plugin
                run_plugin(apk_ctx)
            except Exception:
                pass
            return []
    ...
    def _convert_finding(self, v, index: int) -> PluginFinding:
        ...
        if isinstance(v, dict):
            ...
            return PluginFinding(
                finding_id=f"advanced_ssl_tls_analyzer_{index:03d}",
                title=str(v)[:200],
        ...
        if not cwe:
            tl = (title + " " + desc).lower()
            if any(k in tl for k in ["certificate", "cert", "trust"]):
                cwe = "CWE-295"
            elif any(k in tl for k in ["protocol", "ssl", "tls"]):
                cwe = "CWE-327"
            elif any(k in tl for k in ["cipher", "weak"]):
                cwe = "CWE-327"
...
def create_plugin():
    return AdvancedSSLTLSAnalyzerV2()


__all__ = ["AdvancedSSLTLSAnalyzerV2", "create_plugin"]
